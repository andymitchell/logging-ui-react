 ...
 * can reach. Here that would mean shipping a private copy of React (breaking hooks) or of logging (breaking the
 * `instanceof TraceViewer` check) if one ever stopped being a peer. A vendored vitest is not merely dead weight:
 * a second copy of the runner overwrites `globalThis[Symbol.for('expect-global')]`, silently stealing the
 * consumer runner's per-test state.
 *
...
const RUNNER_IMPORT_ALLOWED_ENTRIES = new Set();
